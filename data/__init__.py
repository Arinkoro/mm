...
def train_dataset_factory(transforms, config):
    ...
    if FashionIQDataset.code() in dataset_code:
        ...
        if len(dataset_clothing_split) == 1:
            raise ValueError(
                "Please specify clothing type for this dataset: fashionIQ_[dress_type]")
...
def create_dataloaders(image_transform, text_transform, configs):
    train_dataset = train_dataset_factory(
        transforms={
            'image_transform': image_transform['train'], 'text_transform': text_transform['train']},
        config=configs)
    test_datasets = test_dataset_factory(
        transforms={
            'image_transform': image_transform['val'], 'text_transform': text_transform['val']},
        config=configs)
    train_val_datasets = test_dataset_factory(
        transforms={
            'image_transform': image_transform['val'], 'text_transform': text_transform['val']},
        config=configs, split='train')
    collate_fn = PaddingCollateFunction(
        padding_idx=AbstractBaseVocabulary.pad_id())
    collate_fn_test = PaddingCollateFunctionTest(
        padding_idx=AbstractBaseVocabulary.pad_id())
    train_dataloader = train_dataloader_factory(
        dataset=train_dataset, config=configs, collate_fn=collate_fn)
    test_dataloaders = test_dataloader_factory(
        datasets=test_datasets, config=configs, collate_fn=collate_fn_test)
